...
class Users {
  ...
  static init({
    ...
    try {
      const users = new Users(); // Initiate EthResolver

      if (ethNetwork === undefined) {
        users.eth = _web.EthResolver.init();
      } else {
        users.eth = _web.EthResolver.init(ethNetwork);
      } // Initiate Swarm connector


      if (swarmHost === undefined) {
        users.bzz = _web.Swarm.init();
      } else {
        users.bzz = _web.Swarm.init(swarmHost);
      }
    ...
    try {
      ...
      if (swarmHash.startsWith('0x00000000000000000000000')) {
        return {
          ...
          bio: 'Not Found',
          avatar: 'Not Found',
          isAdmin: 'Not Found' //throw new Error('Can not retrieve nil values from Swarm')
