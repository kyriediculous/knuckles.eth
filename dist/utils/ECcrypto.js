...
function encryptWithPublicKey(message, pubKey) {
  ...
  pubKey = (0, _secp256k.publicKeyConvert)(new Buffer(pubKey, 'hex'), false).toString('hex');
  pubKey = new Buffer(pubKey, 'hex');
  return (0, _eccrypto.encrypt)(pubKey, Buffer(message)).then(encryptedBuffers => {
    const cipher = {
...
function decryptWithPrivateKey(encrypted, privateKey) {
  ...
  return (0, _eccrypto.decrypt)(new Buffer(twoStripped, 'hex'), encryptedBuffer).then(decryptedBuffer => decryptedBuffer.toString());
}